...
async function generateImageMap() {
  const imagePath = path.join(__dirname, './src/routes/media/components/single/images');
  ...
  for (const dir of dirs) {
    ...
    if (stat.isDirectory()) {
      ...
      imageMap[dir] = imageFiles.filter((file) => file.endsWith('.jpg') || file.endsWith('.png') || file.endsWith('.jpeg'));
    }
  }
